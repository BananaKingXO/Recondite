# Check if last character matches
  ...
  execute if score #StringLib.SuccessCheck StringLib matches 0 if score #StringLib.FindLength StringLib matches 2.. if function recondite:util/stringlib/zprivate/find/reversed/check_word_rest store result storage stringlib:output find[-1] int 1 run return run scoreboard players get #StringLib.Index StringLib
...
# Next loop (Stop once the word can no longer fit, or if it's been found already)
  ...
  function recondite:util/stringlib/zprivate/find/reversed/check_word_start_loop
